...
: DEPTH 0x08 0x4e 0x00 0x02 0x0d 0x22 0x29 ;
...
: (?do) 0x32 0x31 0x01 0x02 0x0d 0x2b 0x3a 0x01 0x00 0x1b 0x29 ;
: I 0x10 0x21 0x29 ;
: J 0x13 0x12 0x10 0x02 0x03 0x21 0x29 ;
: UNLOOP 0x12 0x12 0x29 ;
: (loop) 0x12 0x4d 0x01 0x00 0x0c 0x10 0x02 0x0d 0x2b 0x3a 0x03 0x00 0x12 0x12 0x29 0x1b 0x29 ;
: WAIT 0x00 0x29 ;
...
: MARKER HERE HEADER SWAP 42 C, POSTPONE LITERAL LATEST @ POSTPONE LITERAL ['] (marker) , ['] EXIT , LATEST ! ;
...
: COMPILE, , ;
: AGAIN ['] (branch) , , ;
: COMPILE-WORD 2DUP FIND-WORD ?DUP IF -ROT 2DROP DUP 1- C@ 128 AND IF EXECUTE ELSE , THEN ELSE 2DUP NUMBER 0= IF DROP -ROT 2DROP POSTPONE LITERAL ELSE 2DROP TYPE SPACE 63 EMIT THEN THEN ;
: COUNT DUP 1+ SWAP C@ ;
...
: ACCEPT OVER SWAP OVER + 1- SWAP BEGIN RAWKEY DUP 31 > IF DUP 0x7f = IF EMIT 1- ELSE DUP EMIT OVER C! 1+ OVER MIN THEN ELSE 10 = IF NIP SWAP - EXIT THEN THEN WAIT AGAIN ;
: EXPECT ACCEPT SPAN ! ;
...
: INTERPRET-WORD 2DUP FIND-WORD ?DUP IF -ROT 2DROP EXECUTE ELSE 2DUP NUMBER 0= IF DROP -ROT 2DROP ELSE 2DROP TYPE SPACE 63 EMIT THEN THEN ;
